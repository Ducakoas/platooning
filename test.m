...
% 1. Lưu file có timestamp để lưu trữ lịch sử (như cũ)
    mat_path = fullfile(export_folder, ...
        sprintf('platoon_results_%s.mat',timestamp));
    save(mat_path,'results');
    fprintf('\n[Export] MAT saved:\n  %s\n',mat_path);

    % 2. LƯU THÊM FILE NÀY ĐỂ SCRIPT KHÁC TỰ ĐỘNG LẤY ĐƯỢC NGAY
    latest_path = fullfile(export_folder, 'latest_platoon_results.mat');
    save(latest_path, 'results');
    fprintf('[Export] Latest MAT updated:\n  %s\n',latest_path);
